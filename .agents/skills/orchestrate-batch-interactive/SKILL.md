...
description: Drive a multi-task batch through the engine scheduler on the interactive×Codex lane. Thin supervisor — the DAG/retry/cascade/capacity logic is all in the `orchestrator` engine; this skill only sequences the fan-out.
---

# Batch supervisor — interactive×Codex lane
...
call a model directly and never run `Codex -p`.
...
Require the Codex status-line sensor throughout the loop: configure
...
`interactive:Codex`, zero unattributed. The durable timeline is `events.jsonl`;
