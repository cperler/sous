...
description: Drive one task through the engine on the interactive×Codex lane. Thin supervisor — all deterministic logic lives in the `orchestrator` CLI; this skill only sequences calls.
---

# Supervisor — interactive×Codex lane
...
You never call a model directly and you never run `Codex -p`.
...
Before every model dispatch, require the Codex status-line sensor. Configure
`orchestrator statusline` as this session's `statusLine` command; it retains the existing
5h/7d display and caches Codex's `context_window` payload for the guard. Confirm it
...
1. **next**: `WORK=$(… next --task "$TASK" --util auto --guard-supervisor-context
   --supervisor-resume-command "start a fresh Codex session and invoke
...
be `interactive:Codex` with zero `unattributed`/`off_lane`. A hidden `Codex -p`
