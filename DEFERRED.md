...
| Stronger test-validate (schema- or independent-reviewer-enforced) | code-review 2026-06-24 (test gate) | The shipped gate is fail-OPEN self-report (`tests_meaningful`); a model omitting the field passes, and self-affirmation is weak. A real check = a JSON schema requiring the field (→ SCHEMA_VIOLATION on omission for codex) and/or a separate reviewer dispatch judging test meaningfulness. | **NEW (substance, medium).** The current gate only catches an honest `false`; the strong version pairs with the deferred independent review lens. | When test-meaningfulness matters enough to spend a separate reviewer pass / bundle with the review-lens work |
