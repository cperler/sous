...
- **2026-06-23 — post-Phase-5 gate.** Phases 1–5 + two engine-hardening passes are built
  ...
  built" trigger has fired), **6 kept** (still genuinely unbuilt). Next build candidate:
  **unattended/queue mode** (the only promoted row).
...
| Rich per-stage cost reports (beyond the basic ledger + summary) | cost ledger (OC:2704) | Basic ledger + summary ship in MVP; richer reporting is additive | **KEEP.** Basic ledger + `summary()` + `cost-summary.md` renderer shipped; richer per-stage/session-reuse reporting is still additive. | When cost analysis / the session-reuse-win comparison is run |
