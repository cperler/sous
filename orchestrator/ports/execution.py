"""Execution-runner PORT + registry (target.md §4) — engine-owned, adapter-implemented.

This module lives INSIDE ``orchestrator/`` on purpose (#273): the dependency arrow points
inward, so an execution adapter imports the engine's contract and the engine imports no
adapter. ``adapters/execution/base.py`` remains as a re-export shim; new code should
import from here.
