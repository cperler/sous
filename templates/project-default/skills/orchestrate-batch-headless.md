...
description: Drive a multi-task batch on the headless×claude lane — the default. One `run-headless` driver owns the run and the engine's Scheduler spawns `claude -p` per stage, so no stage prompt or output passes through the session. Use for ordinary batches; the interactive lane is only for watching stages live.
---
